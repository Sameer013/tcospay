<?php
	...
	foreach ($data as $row) {
	 	...
	 	$action = "<div class='flex justify-left items-center'>
					...
					<!--<a class='flex items-center mr-5' id=\"viewBtn\" href='javascript:;' onclick=\"load_data('$ShiftCode')\"
					...
					<a class='flex items-center mr-5' id=\"edit-button\" href='javascript:;' onclick=\"load_data('$ShiftCode')\"
					data-tw-toggle=\"modal\" data-tw-target=\"#header-footer-modal-preview-view\">
					...
					<a class=\"flex items-center text-danger\" href=\"javascript:;\" onclick=\"remove_data('$ShiftCode')\">
	 	...
	 	$object->ShiftCode 	= $row['ShiftCode'];
		$ShiftCode=$row['ShiftCode'];
        $object->InTime	= $row['InTime'];
        $object->OutTime	= $row['OutTime'];
        $object->Night	= $row['Night'];
        $object->OTduringNight	= $row['OTduringNight'];
        $object->LunchTimeFrom	= $row['LunchTimeFrom'];
        $object->LunchTimeTo	= $row['LunchTimeTo'];
