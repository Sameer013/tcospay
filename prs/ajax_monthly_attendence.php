...
try {
    ...
    $sql = "SELECT  sd.empno, e.Name, cl, sd.el, lwop, off_days, med_leave, hdays, attnd, adj, pay_mode, (cl+sd.el+lwop+med_leave+spleave) as absent,sd.spleave, wdays,sd.sheet_id
        FROM sheet_det sd
...
$sql .= " ORDER BY " . $columns[$requestData['order'][0]['column']] . " " . $requestData['order'][0]['dir'] . " LIMIT " . $requestData['start'] . " ," . $requestData['length'];
    ...
    foreach ($data as $row) {
        ...
        $empno = $row['empno'];
        $dpaid =  ($row['cl']+$row['off_days']+($row['med_leave']/2)+$row['attnd']+$row['hdays']+$row['spleave']+$row['el']);
        $wdays = $daysInMonth - ($row['off_days']+$row['hdays']) ;

        $action = "<div class='flex justify-left items-center'>
                        <a class='flex items-center mr-5' id=\"edit-button\" href='javascript:;' onclick=\"load_data('$empno',$sheet_id)\"
                        ...
                        <a class=\"flex items-center text-danger\" href=\"javascript:;\" onclick=\"remove_data('$empno',$sheet_id)\">
