<?php
	...
	$sql = "SELECT cm.scdate, cm.scedate, c.DESCR, cm.ShiftCode,cm.catcode FROM catshiftchange cm
	JOIN catmast c on cm.catcode=c.cat_code";
	...
	foreach ($data as $row) {
	 	...
	 	$action = "<div class='flex justify-left items-center'>
					...
					<a class='flex items-center mr-5' id=\"edit-button\" href='javascript:;' onclick=\"load_data('$scdate','$catcode')\"
					...
					<a class=\"flex items-center text-danger\" href=\"javascript:;\" onclick=\"remove_data('$scdate','$catcode')\">
	 	...
	 	$object->scdate 	= $row['scdate'];
		$scdate=$row['scdate'];
        $object->scedate	= $row['scedate'];
