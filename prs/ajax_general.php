<?php
	...
	foreach ($data as $row) {
	 	...
	 	$action = "<div class='flex justify-left items-center'>
					...
					<!--<a class='flex items-center mr-5' id=\"viewBtn\" href='javascript:;' onclick=\"load_data('$empcode')\"
					...
					<a class='flex items-center mr-5' id=\"edit-button\" href='javascript:;' onclick=\"load_data('$empcode')\"
					data-tw-toggle=\"modal\" data-tw-target=\"#header-footer-modal-preview-view\">
					...
					<a class=\"flex items-center text-danger\" href=\"javascript:;\" onclick=\"remove_data('$empcode')\">
	 	...
	 	$object->empcode 	= $row['EMPNO'];
        $object->comp_id	= $row['comp_id'];
	 	$object->name	= $row['NAME'];
	 	$object->status = $row['STATE'];
	 	$object->gender = $row['SEX'];
	 	$object->address	= $row['ADDRESS'];
	 	$object->location	= $row['ADDRESS'];
	 	$object->phone	= $row['PHONE'];
	 	$object->mobile	= $row['PHONE1'];
	 	$object->category	= $row['DISPORDER'];
	 	$object->mar_status	= $row['mar_stat'];
	 	$object->pan	= $row['PAN'];
	 	$object->cat_code	= $row['PAN'];
	 	$object->dsgcode	= $row['PAN'];
	 	$object->dob	= $row['DOB'];
	 	$object->doj	= $row['DOJ'];
	 	$object->dor	= $row['DOR'];
	 	$object->doc	= $row['DOC'];
	 	$object->card_no	= $row['DOC'];
