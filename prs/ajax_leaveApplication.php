...
try {
    ...
    if ($userId == 'Admin' || $userId == 'Super Admin') {
        $sql = "SELECT t.*, e.NAME from leaveapply t JOIN empmast e ON t.empno = e.empno WHERE t.status is null";
    } else {
        $sql = "SELECT t.*, e.NAME from leaveapply t LEFT JOIN empmast e ON t.empno = e.empno WHERE t.empno=$userId and t.status IS null";
    }
    ...
    foreach ($data as $row) {
        ...
        if (!empty($row['DOCUMENT'])) {
            $object->document = "<a href='view_document.php?slno=" . $row['slno'] . "' target='_blank'>Open</a>";
        } else {
            $object->document = '';
        }
