<?php
	...
	foreach ($data as $row) {
	 	...
	 	$action = "<div class='flex justify-left items-center'>
					...
					<!--<a class='flex items-center mr-5' id=\"viewBtn\" href='javascript:;' onclick=\"load_data('$LeaveCode')\"
					...
					<a class='flex items-center mr-5' id=\"edit-button\" href='javascript:;' onclick=\"load_data('$LeaveCode')\"
					data-tw-toggle=\"modal\" data-tw-target=\"#header-footer-modal-preview-view\">
					...
					<a class=\"flex items-center text-danger\" href=\"javascript:;\" onclick=\"remove_data('$LeaveCode')\">
		...
		$LeaveCode=$row['LeaveCode'];
        $object->LeaveName	= $row['LeaveName'];
	 	$object->PaidLeave	= $row['PaidLeave'];
