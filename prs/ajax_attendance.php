<?php
	...
	foreach ($data as $row) {
	 	...
	 	$action = "<div class='flex justify-left items-center'>
					...
					<!--<a class='flex items-center mr-5' id=\"viewBtn\" href='javascript:;' onclick=\"load_data('$mac_id')\"
					...
					<a class='flex items-center mr-5' id=\"edit-button\" href='javascript:;' onclick=\"load_data('$mac_id')\"
					data-tw-toggle=\"modal\" data-tw-target=\"#header-footer-modal-preview-view\">
					...
					<a class=\"flex items-center text-danger\" href=\"javascript:;\" onclick=\"remove_data('$mac_id')\">
	 	...
	 	$object->mac_id 	= $row['mac_id'];
		$mac_id=$row['mac_id'];
        $object->mac_descr	= $row['mac_descr'];
	 	$object->ipaddr	= $row['ipaddr'];
